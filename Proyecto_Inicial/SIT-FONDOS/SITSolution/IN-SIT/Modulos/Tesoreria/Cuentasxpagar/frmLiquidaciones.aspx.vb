...
Partial Class Modulos_Tesoreria_Cuentasxpagar_frmLiquidaciones
    Inherits BasePage
    ...
    'Renovacion
    Private Sub CargarTransferenciasBCR()
        Dim dt As DataTable = oOrdenInversionBM.TransferenciaBCR(ddlPortafolio.SelectedValue, UIUtility.ConvertirFechaaDecimal(txtFechaInicio.Text))
        If dt.Rows.Count > 0 Then
            gvBCR.DataSource = dt
        Else
            gvBCR.DataSource = Nothing
        End If
        gvBCR.DataBind()
    End Sub
    Private Sub CargarRenovacion()
        ...
        If (sCategoria.Equals("OR")) Then
            dt = oOrdenInversionBM.VencimientoRenovacion(ddlPortafolio.SelectedValue, hdCodigoTrecero.Value, UIUtility.ConvertirFechaaDecimal(txtFechaInicio.Text))
        Else
            dt = oOrdenInversionBM.VencimientosdelDiaDPZ(ddlPortafolio.SelectedValue, hdCodigoTrecero.Value, UIUtility.ConvertirFechaaDecimal(txtFechaInicio.Text))
        End If
            ...
            If fila.RowType = DataControlRowType.DataRow Then
                ...
                If Not chkSelect Is Nothing Then
                    If chkSelect.Checked Then
                        sCategoria = fila.Cells.Item(16).Text
                        Indice += 1
                        If Indice > 1 Then
                            Return False
                        End If
    ...
    Function ValidaRenovacion() As Boolean
        ...
        End If
    End Function
    Function ValidaBCR() As Boolean
        Dim correlativo As Integer = 0
        For Each fila As GridViewRow In gvBCR.Rows
            If fila.RowType = DataControlRowType.DataRow Then
                Dim chkSelect As System.Web.UI.WebControls.CheckBox = CType(fila.FindControl("chkSelect"), System.Web.UI.WebControls.CheckBox)
                If Not chkSelect Is Nothing Then
    ...
    Sub InsertarRenovacion(CodigoOrden As String)
        ...
        oOrdenInversionBM.Libera_Renovacion(CodigoOrden) 'Or ddlModeloCarta.SelectedValue = "VNOR"
        CodRelacion = oOrdenInversionBM.Insertar_DPZRenovacionCabecera(CodigoOrden, IIf(ddlModeloCarta.SelectedValue = "VNOR", "101", "3"), "", UIUtility.ConvertirFechaaDecimal(txtFechaInicio.Text))
        For Each fila As GridViewRow In gvRenovacion.Rows
            If fila.RowType = DataControlRowType.DataRow Then
                Dim chkSelect As System.Web.UI.WebControls.CheckBox = CType(fila.FindControl("chkSelect"), System.Web.UI.WebControls.CheckBox)
                ...
                If Not chkSelect Is Nothing Then
                    If chkSelect.Checked Then
                        oOrdenInversionBM.Insertar_DPZRenovacionDetalle(CodRelacion, lbCodigo.Text, "4", UIUtility.ConvertirFechaaDecimal(txtFechaInicio.Text))
                    End If
                End If
            End If
        Next
    End Sub
    Sub InsertarBCR(CodigoOrden As String)
        oOrdenInversionBM.Libera_Renovacion(CodigoOrden)
        Dim CodRelacion As Integer
        CodRelacion = oOrdenInversionBM.Insertar_DPZRenovacionCabecera(CodigoOrden, "3", "", UIUtility.ConvertirFechaaDecimal(txtFechaInicio.Text))
        For Each fila As GridViewRow In gvBCR.Rows
            If fila.RowType = DataControlRowType.DataRow Then
                Dim chkSelect As System.Web.UI.WebControls.CheckBox = CType(fila.FindControl("chkSelect"), System.Web.UI.WebControls.CheckBox)
                Dim lbCodigo As System.Web.UI.WebControls.Label = CType(fila.FindControl("lbCodigo"), System.Web.UI.WebControls.Label)
                If Not chkSelect Is Nothing Then
                    If chkSelect.Checked = True Then
                        oOrdenInversionBM.Insertar_DPZRenovacionDetalle(CodRelacion, lbCodigo.Text, "BCRE", UIUtility.ConvertirFechaaDecimal(txtFechaInicio.Text))
                    End If
                End If
            End If
        Next
...
#Region "CargarDatos"
    Sub CargaBancoDifRenovacion()
        ...
        UIUtility.CargarIntermediariosOISoloBancos(ddlBancoGlosaIngreso)
    End Sub
    ...
    Private Sub CargarBancoDestino(Optional ByVal sCodigoMercado As String = "", Optional ByVal sCodigoPortafolioSBS As String = "", Optional ByVal sCodigoMoneda As String = "", Optional ByVal enabled As Boolean = True)
        ...
        If enabled Then
            Dim oBanco As New TercerosBM
            Dim dsBanco As DataSet = oBanco.SeleccionarBancoPorMercPortMone(String.Empty, ddlPortafolio.SelectedValue, sCodigoMoneda)
            dtBancos = Distinct(dsBanco.Tables(0), "Descripcion")
    ...
    Private Sub CargarOperacion(Optional ByVal enabled As Boolean = True)
        If enabled Then
            ...
            Dim dt As DataTable = oOperacion.SeleccionarPorCodigoTipoOperacion("", "S", "20").Tables(0)
            HelpCombo.LlenarComboBox(ddlOperacion, dt, "CodigoOperacion", "Descripcion", True, "SELECCIONE")
        Else
    ...
    Private Sub CargarIntermediario(Optional ByVal sMercado As String = "", Optional ByVal enabled As Boolean = True)
        ...
        If enabled Then
            ...
            Dim dt As DataTable = oIntermediario.SeleccionarPorFiltroMercado(ParametrosSIT.CLASIFICACIONTERCERO_INTERMEDIARIO, "", sCodigoPais).Tables(0)
    ...
    Private Sub CargarMoneda(Optional ByVal enabled As Boolean = True)
        If enabled Then
            ...
            Dim dt As DataTable = oMoneda.GetMonedaMercadoPortafolio("", ddlPortafolio.SelectedValue).Tables(0)
            'OT 10238 Fin
            HelpCombo.LlenarComboBox(ddlMoneda, dt, "CodigoMoneda", "Descripcion", False)
    ...
    Private Sub CargarCuentasDestino(ByVal codTercero As String, ByVal CodigoClaseCuenta As String, ByVal CodigoPortafolio As String, Optional ByVal enabled As Boolean = True)
        ...
        If enabled Then
            Dim oBanco As New TercerosBM
            Dim dsBanco As DataSet = oBanco.SeleccionarBancoPorMercPortMone(String.Empty, ddlPortafolio.SelectedValue, Me.hidCodigoMonedaDestino.Value)
            dtBancos = dsBanco.Tables(0)
        ...
        If enabled Then
            ...
            If pnlDestinoDivisas.Attributes("class") Is Nothing Then
                If ddlMoneda.SelectedValue = String.Empty Then
                    codigMoneda = hidCodigoMonedaOrigen.Value
                Else
                    codigMoneda = ddlMoneda.SelectedValue
                End If
            Else
                If pnlDestinoDivisas.Attributes("class").ToUpper().Equals("HIDDEN") Then
...
#Region "Eventos de la Página"
    Private Sub CargarGrilla()
        ...
        Dim dsCuentas As DataSet = New CuentasPorCobrarBM().SeleccionarPorFiltro(dsCuentasPorCobrar, fechaIni, fechaFin, DatosRequest, LiquidaFechaAnt)
        Dim view As DataView = dsCuentas.Tables(0).DefaultView
        view.RowFilter = "Estado <> 'E'"
        dgLista.DataSource = view
        dgLista.DataBind()
        Session("GrillaCuentasPagar") = Nothing
        Dim dtTablaPagar As New DataTable
        dtTablaPagar = dsCuentas.Copy.Tables(0)
        Session("GrillaCuentasPagar") = dtTablaPagar
        Me.ddlContacto.SelectedIndex = 0
        If ddlModeloCarta.DataSource Is Nothing Then
        ...
        txtDescripcion.Text = ""
        txtImporte.Text = ""
        ddlNroCuenta.SelectedIndex = 0
        ddlBanco.SelectedIndex = 0
        ddlClase.SelectedIndex = 0
        pnlDestinoDivisas.Attributes.Add("class", "hidden")
        lbContador.Text = MostrarResultadoBusqueda(dsCuentas.Tables(0))
        ddlContactoIntermediario.Items.Clear()
        ddlNroCuenta.Items.Clear()
        UIUtility.InsertarElementoSeleccion(ddlNroCuenta)
        If ddlPortafolio.SelectedValue = PORTAFOLIO_MULTIFONDOS And ddlMoneda.SelectedValue = "NSOL" Then
            dgLista.Columns(7).HeaderText = "Importe" & vbCrLf & "Soles"
        Else
            dgLista.Columns(7).HeaderText = "Importe"
        End If
        dgLista.DataBind()
        'pnBancoRenovacion.Visible = False
    End Sub
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            If Not Page.IsPostBack Then
                ...
                CargarPortafolio()
                txtPago.Text = UIUtility.ConvertirDecimalAStringFormatoFecha(ObtenerFechaMaximaNegocio())
                CargarOperacion()
                'OT 10238 - 07/04/2017 - Carlos Espejo
                'Descripcion: Se cargan los intermediarios sin el mercado
    ...
    End Sub
    Private Sub btnPagoParcial_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPagoParcial.Click
        ...
        Dim codMoneda As String = selectedRow.Cells(15).Text
        Dim codImporte As String = selectedRow.Cells(7).Text.Replace(UIUtility.DecimalSeparator, ".")
        Dim nroOperacion As String = selectedRow.Cells(5).Text
        Response.Redirect("frmPagoParcial.aspx?codPortafolio=" & codPortafolio & "&codMoneda=" & codMoneda & "&Importe=" & codImporte & "&NumeroOp=" & nroOperacion)
    End Sub
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        If ddlPortafolio.SelectedValue = "" Then
            AlertaJS("El campo portafolio es obligatorio para realizar la búsqueda")
            Exit Sub
        End If
    ...
    Private Sub CargarDatosBanco()
        CargarClaseCuenta(True, ddlBanco.SelectedValue)
        CargarDetallePortafolio(ddlBanco.SelectedValue, ddlClase.SelectedValue, ddlPortafolio.SelectedValue)
        CargarContactos()
    End Sub
    ...
    Private Function VerificaDatoAgrupado(ByVal nIndice As Integer) As Boolean
        ...
        Return bValor
    End Function
    Private Function VerificaMercadoExtranjero(ByVal nIndice As Integer) As Boolean
        Dim bValor As Boolean = True
        bValor = True
        Dim sMercadoBanco As String = New EntidadBM().Seleccionar(ddlBanco.SelectedValue, DatosRequest).Tables(0).Rows(0)("CodigoMercado")

        If sMercadoBanco <> ParametrosSIT.MERCADO_EXTERIOR Then
            bValor = False
            Return bValor
        End If
        Return bValor
    ...
    Private Sub ConfirmaSaldosBancarios()
        ...
        With script
            ...
            .Append("   { document.getElementById('" + btnLiquidar.ClientID + "').onclick = ''; document.getElementById('" + btnLiquidar.ClientID + "').click(); } ")
            .Append("   else ")
        ...
        Try
            ...
            If VerificaAgrupacion() Then 'Verifica si esta agrupado
                If Not VerificaDatoAgrupado(9) Then    'para Portafolio
                    ...
                    Exit Sub
                End If
                'OT10952 - 17/11/2017 - Ian Pastor M. Se debe de liquidar masivamente sin importar el intermediario
                'If Not VerificaDatoAgrupado(11) Then    'para Intermediario
                '    AlertaJS(ObtenerMensaje("ALERT143"))
                '    Exit Sub
                'End If
                'OT10952 - Fin
                ...
                If Not VerificaDatoAgrupado(20) Then    'para Codigo Renta
                    ...
                    Exit Sub
                End If
                If ddlModeloCarta.SelectedValue = "CO03" Then
                    If ValidaRenovacion() = False Then
                        AlertaJS("La carta de renovación requiere que se seleccione algun vencimiento de la lista.")
                        Exit Sub
                    End If
                End If
                If ddlModeloCarta.SelectedValue = "CO04" Then
                    If ValidaBCR() = False Then
                        AlertaJS("La carta de renovación BCR requiere que se seleccione alguna trasferencia BCR de la lista.")
                        Exit Sub
                    End If
                End If
                If ddlModeloCarta.SelectedValue = "VNOR" Then 'Renovacion - Operación de Reporte
                    If ValidaRenovacion() = False Then
                        AlertaJS("La carta de renovación requiere que se seleccione algun vencimiento de la lista.")
                        Exit Sub
                    End If
                ...
                If oDSNroCartaPortafolio.Tables.Count > 0 Then
                    If Not oDSNroCartaPortafolio.Tables(0).Rows(0).Item(0) Is DBNull.Value Then 'RGF 20080723
            ...
            If (pnlDestinoDivisas.Attributes("class") = "hidden") Then
                If Not (ddlPortafolio.SelectedValue = PORTAFOLIO_MULTIFONDOS And ddlMoneda.SelectedValue = "NSOL") Then
                ...
                End If
            End If
            If Not (pnlDestinoDivisas.Attributes("class") = "hidden") Then
                ...
                End If
            End If
            Dim tipoPagoAbono As String = New ParametrosGeneralesBM().Listar(ParametrosSIT.CODIGO_TIPOPAGO_ABONO, DatosRequest).Select("Nombre='Abono'")(0)("Valor")
            Dim bancoContinental As String = New ParametrosGeneralesBM().Listar(ParametrosSIT.CODIGO_BANCO_CONTINENTAL, DatosRequest).Select("Nombre='BancoContinental'")(0)("Valor")
            ...
            If hdValSaldo.Value = "0" Then
                ...
                roCuentaEconomica.NumeroCuenta = ddlNroCuenta.SelectedValue.Split(",")(1)
                ...
                If oSBDS.Tables(0).Rows.Count > 0 Then
                    sSaldo = oSBDS.Tables(0).Rows(0).Item("SaldoDisponible")
                Else
                    sSaldo = "0"
                End If
                Dim saldo As Decimal = Decimal.Parse(sSaldo.Replace(".", UIUtility.DecimalSeparator()))
                If Not txtImporte.Text.Trim.Equals("") And IsNumeric(txtImporte.Text) Then
                    If saldo < Decimal.Parse(txtImporte.Text.Replace(".", UIUtility.DecimalSeparator())) Then
                        hdValSaldo.Value = "1"
            ...
            For Each row As GridViewRow In dgLista.Rows
                If CType(row.FindControl("chbConfirmar"), CheckBox).Checked Then
                    ...
                    opCaja.BancoGlosaDestino = ddlBancoGlosaIngreso.SelectedValue
                    ...
                    If (pnlDestinoDivisas.Attributes("class") Is Nothing) Then
                        ...
                        End If
                        dsOpCaja.OperacionCaja.AddOperacionCajaRow(opCaja)
                        oCuentasPorCobrar.Liquidar(dsOpCaja, sNroCartaPortafolio, CodigoContacto, DatosRequest, ddlBancoDestino.SelectedValue, ddlBanco.SelectedValue, ddlCuentaDestino.SelectedValue.Split(",")(1), Agrupado, txtObservacionCarta.Text, String.Empty)
                    Else
                        opCaja.Importe = Decimal.Parse(row.Cells(7).Text.Replace(".", UIUtility.DecimalSeparator()))
                        dsOpCaja.OperacionCaja.AddOperacionCajaRow(opCaja)
                        Dim sCodigoRelacion As String = ObtieneCodigoRenovacion()
                        oCuentasPorCobrar.Liquidar(dsOpCaja, sNroCartaPortafolio, CodigoContacto, DatosRequest, String.Empty, String.Empty, String.Empty, Agrupado, txtObservacionCarta.Text, sCodigoRelacion)
                    End If
                    If ddlModeloCarta.SelectedValue = "CO03" Or ddlModeloCarta.SelectedValue = "VNOR" Then
                        InsertarRenovacion(row.Cells(1).Text)
                    ElseIf ddlModeloCarta.SelectedValue = "CO04" Then
                        InsertarBCR(row.Cells(1).Text)
                    End If
                    'INICIO | ZOLUXIONES | RCE | ProyFondosII - RF019 - Se agrega nueva validación para validar suscripción de fondos Capestr | 01/08/18 
                    If ddlPortafolio.SelectedItem.Text.Contains("CAPESTR") And row.Cells(24).Text = "50" Then
                        rpta = oCuentasPorCobrar.SuscripcionFondos_Automatico(DatosRequest, opCaja.CodigoPortafolioSBS, hidCodigoMonedaOrigen.Value, opCaja.NumeroCuenta, opCaja.Importe, ddlBanco.SelectedValue, ddlClase.SelectedValue, opCaja.FechaPago)
                    End If
                    'FIN | ZOLUXIONES | RCE | ProyFondosII - RF019 - Se agrega nueva validación para validar suscripción de fondos Capestr | 01/08/18 
                End If
            Next
            ...
            ElseIf rpta = 1 Then
                AlertaJS(ObtenerMensaje("ALERT177"))
            Else
                AlertaJS(ObtenerMensaje("ALERT154"))
                AlertaJS(ObtenerMensaje("ALERT178"))
            End If
            LimpiarRenovacion()
            CargarGrilla()
        Catch ex As Exception
    ...
    Public Sub chbConfirmar_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not validarAgrupacion(selectedRow, importeTotal) Then
            If Not selectedRow Is Nothing Then
                ...
                hdIntermediario.Value = HttpUtility.HtmlDecode(selectedRow.Cells(11).Text)
                ...
                codigoOrden = selectedRow.Cells(17).Text
                dt = ordenInverscionBM.GetOrdenInversionDivisas(ddlPortafolio.SelectedValue, codigoOrden)

                Dim delivery As String = ""
                If dt.Rows.Count > 0 Then
                    If Not dt.Rows(0)("Delibery") Is Nothing Then
                        delivery = dt.Rows(0)("Delibery").ToString()
                    End If
                End If

                If (codigoOperacion = "65" Or codigoOperacion = "66" Or ((codigoOperacion = "93" Or codigoOperacion = "94") And delivery = "S")) Then

                    If (codigoOperacion = "93" Or codigoOperacion = "94") Then
                        Me.txtImporteDestino.Text = Convert.ToDecimal(dt.Rows(0)("MontoOperacion"))
                    Else
                        Me.txtImporteDestino.Text = Convert.ToDecimal(dt.Rows(0)("MontoDestino"))
                    End If

                    txtDescripcion.Text = lblMoneda.Text
                    lblMoneda.Text = txtDescripcion.Text
                    lblMonedaDestino.Text = Convert.ToString(dt.Rows(0)("CodigoMonedaDestino"))
                    txtFechaVcto.Text = HttpUtility.HtmlDecode(selectedRow.Cells(4).Text).Trim
                    codMoneda = Convert.ToString(dt.Rows(0)("CodigoMonedaDestino"))
                Else
                    If (codigoOperacion = "94") Then
                        If dt.Rows(0)("Delibery") = "S" Then
                            lblMoneda.Text = Convert.ToString(dt.Rows(0)("Moneda"))
                            txtImporte.Text = Convert.ToDecimal(dt.Rows(0)("MontoOperacion"))
                        Else
                    ...
                    Else
                        If lblMoneda.Text = "" Then
                            lblMoneda.Text = ddlMoneda.SelectedItem.Text 'Cambio Ipastor: Para que no setee la descripcion de la moneda seleccionada en la grilla
                        End If
                        txtImporte.Text = HttpUtility.HtmlDecode(selectedRow.Cells(7).Text)
                    End If
                    txtDescripcion.Text = HttpUtility.HtmlDecode(selectedRow.Cells(6).Text).Trim
                    txtFechaVcto.Text = HttpUtility.HtmlDecode(selectedRow.Cells(4).Text).Trim
                    codMoneda = selectedRow.Cells(15).Text
                End If
                ...
                Dim dtContacto As DataTable = objContacto.ListarContactoPorTercerosTesoreria(selectedRow.Cells(22).Text).Tables(0)
                HelpCombo.LlenarComboBox(ddlContactoIntermediario, dtContacto, "CodigoContacto", "DescripcionContacto", False)
        ...
        If Not selectedRow Is Nothing Then
            ...
            If (codigoOperacion = "65" Or codigoOperacion = "66") Then
                pnlDestinoDivisas.Attributes.Remove("class")
                For Each row As GridViewRow In dgLista.Rows
                    If CType(row.FindControl("chbConfirmar"), CheckBox).Checked Then
                        ...
                        If Not (dt Is Nothing) Then
                            If Not dt.Rows(0)("MontoDestino") Is DBNull.Value Then
                                totalDestino = totalDestino + Convert.ToDecimal(dt.Rows(0)("MontoOrigen"))
                            End If
                            hidCodigoMonedaDestino.Value = Convert.ToString(dt.Rows(0)("CodigoMonedaDestino"))
                            hidCodigoMonedaOrigen.Value = Convert.ToString(dt.Rows(0)("CodigoMonedaOrigen"))
                            CargarBancoDestino(String.Empty, ddlPortafolio.SelectedValue, Me.hidCodigoMonedaDestino.Value, True)
                            CargarBanco("", ddlPortafolio.SelectedValue, hidCodigoMonedaOrigen.Value, True)
                            CargarClaseCuenta(True, ddlBanco.SelectedValue)
                            CargarDetallePortafolio(ddlBanco.SelectedValue, String.Empty, ddlPortafolio.SelectedValue, True)
                        End If
                        If Not (dtDiv Is Nothing) Then
                            If dtDiv.Rows.Count > 0 Then
                                ...
                                CargarCuentasDestino(String.Empty, String.Empty, String.Empty, True)
                                ...
                                BindingModeloCarta("66", "")
                                ddlModeloCarta.SelectedValue = drBE.CodigoModelo
                            End If
                        End If
                        ddlBanco_SelectedIndexChanged(Nothing, Nothing) 'HSP 20150902.
                        ddlBancoDestino_SelectedIndexChanged(Nothing, Nothing) 'HSP 20150902.​
                    End If
                Next
                Me.txtImporte.Text = totalDestino.ToString()
                If (Me.ddlContacto.Items.Count < 1) Then
                    Me.ddlContacto.Items.Insert(0, New ListItem("--SELECCIONE--", ""))
                End If
                    ...
                    If CType(row.FindControl("chbConfirmar"), CheckBox).Checked Then
                        ...
                        If Not (dt Is Nothing) Then
                            If Not dt.Rows(0)("MontoOperacion") Is DBNull.Value Then
                                totalDestino = totalDestino + Convert.ToDecimal(dt.Rows(0)("MontoCancelar"))
                            End If
        ...
        End If
    End Sub
    Private Sub ddlMonedaCuenta_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        CargarDetallePortafolio(ddlBanco.SelectedValue, ddlClase.SelectedValue, "")
    End Sub
#End Region
#Region "Metodos de control"
    Private Function validarAgrupacion(ByRef diCuenta As GridViewRow, Optional ByRef total As Decimal = 0) As Boolean
        ...
        Dim codigoOperacion As String = String.Empty
        Dim codigoOperacionID As String = String.Empty
        For Each row As GridViewRow In dgLista.Rows
            If CType(row.FindControl("chbConfirmar"), CheckBox).Checked Then
                count = count + 1
                diCuenta = row
                ...
                If (UIUtility.ConvertirFechaaDecimal(Me.txtFechaInicio.Text) <> UIUtility.ConvertirFechaaDecimal(row.Cells(4).Text)) Then
                    If (codigoOperacionID <> "3") Then
                        ...
                        Me.btnAceptarFecha.Visible = True
                    End If
                End If
                If (operacionesSeleccionandas.Count = 0) Then
                    operacionesSeleccionandas.Add(codigoOperacion)
                    If HttpUtility.HtmlDecode(row.Cells(6).Text).Trim <> "" Then
                        'Me.BindingModeloCarta(row.Cells(19).Text, IIf(HttpUtility.HtmlDecode(row.Cells(6).Text).Substring(0, 11) = "FORWARD-NOM", "N", IIf(row.Cells(6).Text.Substring(0, 11) = "FORWARD-DEL", "S", "")), "S")
                        Me.BindingModeloCarta(row.Cells(19).Text, row.Cells(17).Text)
                    End If
                Else
...
#End Region
    Private Sub ddlMoneda_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ddlMoneda.SelectedIndexChanged
        ...
        dgLista.DataBind()
        CargarBanco("", ddlPortafolio.SelectedValue, ddlMoneda.SelectedValue)
        CargarDetallePortafolio("", "", "", False)
        CargarClaseCuenta(False, "")
        CargarDetallePortafolio(ddlBanco.SelectedValue, ddlClase.SelectedValue, ddlPortafolio.SelectedValue, True)
    End Sub
    ...
    Private Sub btnImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImprimir.Click
        ...
        EjecutarJS(UIUtility.MostrarPopUp("../Reportes/frmVisorTesoreria.aspx?LiquidacionesPagar=" & Liquidaciones, "no", 1100, 850, 40, 150, "no", "yes", "yes", "yes"), False)
    End Sub
    Private Sub btnAceptarFecha_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAceptarFecha.Click
        ...
        If Convert.ToDateTime(txtFechaVcto.Text) < Convert.ToDateTime(txtFechaInicio.Text) And hdIndLiqAntFondo.Value <> "1" Then  'HDG OT 64767 20120222
        ...
        Try
            If txtFechaVcto.Text <> "" Then
                ...
                If (UIUtility.ConvertirFechaaDecimal(Me.txtFechaInicio.Text) = UIUtility.ConvertirFechaaDecimal(txtFechaVcto.Text)) Or hdIndLiqAntFondo.Value = "1" Then    'HDG OT 64767 20120222
        ...
        End Try
    End Sub
    Private Sub ddlOperacion_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ddlOperacion.SelectedIndexChanged
        If (ddlOperacion.SelectedIndex = -1) Then
            Return
        End If
        BindingModeloCarta(ddlOperacion.SelectedValue, "") 'OT12012
    End Sub
    Private Sub BindingModeloCarta(ByVal codigoOperacion As String, ByVal CodigoOrden As String) 'OT12012
        Dim operacion As New OperacionBM
        Try
            Dim dt As DataTable = operacion.ListaModeloCartaOperacion(codigoOperacion, CodigoOrden)
    ...
    Private Sub ddlModeloCarta_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlModeloCarta.SelectedIndexChanged
        ...
        If ddlModeloCarta.SelectedValue = "CO03" Or ddlModeloCarta.SelectedValue = "VNOR" Then
            If ValidaTerceroOI() Or ddlModeloCarta.SelectedValue = "VNOR" Then
        ...
        ElseIf ddlModeloCarta.SelectedValue = "CO04" Then
            pnBCR.Visible = True
            pnRenovacion.Visible = False
            CargarTransferenciasBCR()
        Else
            pnBCR.Visible = False
            pnRenovacion.Visible = False
        End If
        If (ddlContacto.Items.Count < 1) Then
            ddlContacto.Items.Insert(0, New ListItem("--SELECCIONE--", ""))
        End If
    End Sub
    Protected Sub dgLista_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles dgLista.PageIndexChanging
        ...
        CargarGrilla()
    End Sub
    Protected Sub btnSaldos_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaldos.Click
    ...
    End Sub
    Function ObtieneCodigoRenovacion() As String
        Dim sCodigoRelacion As String = ""
        For Each fila As GridViewRow In gvRenovacion.Rows
            If fila.RowType = DataControlRowType.DataRow Then
                Dim chkSelect As System.Web.UI.WebControls.CheckBox = CType(fila.FindControl("chkSelect"), System.Web.UI.WebControls.CheckBox)
                Dim lbCodigo As System.Web.UI.WebControls.Label = CType(fila.FindControl("lbCodigo"), System.Web.UI.WebControls.Label)
                If Not chkSelect Is Nothing Then
                    If chkSelect.Checked Then
                        sCodigoRelacion = lbCodigo.Text
                    End If
                End If
            End If
        Next
        Return sCodigoRelacion
    End Function
End Class