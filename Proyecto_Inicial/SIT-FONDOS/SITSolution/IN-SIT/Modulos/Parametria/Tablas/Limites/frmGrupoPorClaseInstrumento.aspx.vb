...
Imports System.Data
Partial Class Modulos_Parametria_Tablas_Limites_frmGrupoPorClaseInstrumento
    Inherits BasePage
    ...
    Sub Limpiar()
        ...
        Dim dt As DataTable = oLimParametria.GrupoClaseInstrumento_Grupo("", "", "0")
        HelpCombo.LlenarListBox(lbxValores, dt, "CATEGORIA", "Descripcion", False)
    End Sub
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            If Session("Codigo") Is Nothing Then
                Session("Codigo") = ""
            End If
            Dim dtD As DataTable = oLimParametria.GrupoClaseInstrumento_Seleccionar(Session("Codigo").ToString)
            If dtD.Rows.Count > 0 Then
                txtcodigoclase.Text = dtD(0)("CodigoClaseInstrumento").ToString
                txtDescripcion.Text = dtD(0)("Descripcion").ToString
                ddlSituacion.SelectedValue = dtD(0)("Situacion").ToString
            End If
            Dim dt As DataTable = oLimParametria.GrupoClaseInstrumento_Grupo(Session("Codigo").ToString, txtDesVal.Text, "0")
            Dim dt2 As DataTable = oLimParametria.GrupoClaseInstrumento_Grupo(Session("Codigo").ToString, "", "1")
            HelpCombo.LlenarListBox(lbxValores, dt, "CATEGORIA", "Descripcion", False)
            HelpCombo.LlenarListBox(lbxSeleccionValores, dt2, "CATEGORIA", "Descripcion", False)
        End If
    End Sub
    Protected Sub btnAceptar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAceptar.Click
        Try
            If txtcodigoclase.Text = "" Then
            ...
            Else
                oLimParametria.GrupoClaseInstrumento_Borrar(txtcodigoclase.Text)
                For Each it As ListItem In lbxSeleccionValores.Items
                    oLimParametria.GrupoClaseInstrumento_Insertar(txtcodigoclase.Text, txtDescripcion.Text, it.Value, ddlSituacion.SelectedValue, DatosRequest)
                Next
