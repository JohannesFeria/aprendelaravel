...
Imports System.Data
Partial Class Modulos_Parametria_Tablas_Limites_frmGrupoPorEntidad
    Inherits BasePage
    ...
    Sub Limpiar()
        ...
        Dim dt As DataTable = oLimParametria.GrupoPorEntidad_Grupo("", "", "0")
        HelpCombo.LlenarListBox(lbxValores, dt, "CodigoEntidad", "Descripcion", False)
    End Sub
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            If Session("Codigo") Is Nothing Then
                Session("Codigo") = ""
            End If
            Dim dtD As DataTable = oLimParametria.GrupoPorEntidad_Seleccionar(Session("Codigo").ToString)
            If dtD.Rows.Count > 0 Then
                txtcodigogrupo.Text = dtD(0)("CodigoGrupoEntidad").ToString
                txtDescripcion.Text = dtD(0)("Descripcion").ToString
                ddlSituacion.SelectedValue = dtD(0)("Situacion").ToString
            End If
            Dim dt As DataTable = oLimParametria.GrupoPorEntidad_Grupo(Session("Codigo").ToString, txtDesVal.Text, "0")
            Dim dt2 As DataTable = oLimParametria.GrupoPorEntidad_Grupo(Session("Codigo").ToString, "", "1")
            HelpCombo.LlenarListBox(lbxValores, dt, "CodigoEntidad", "Descripcion", False)
            HelpCombo.LlenarListBox(lbxSeleccionValores, dt2, "CodigoEntidad", "Descripcion", False)
        End If
    End Sub
    Protected Sub btnAceptar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAceptar.Click
        Try
            ...
            Else
                oLimParametria.GrupoPorEntidad_Borrar(txtcodigogrupo.Text)
                For Each it As ListItem In lbxSeleccionValores.Items
                    oLimParametria.GrupoPorEntidad_Insertar(txtcodigogrupo.Text, txtDescripcion.Text, it.Value, ddlSituacion.SelectedValue, DatosRequest)
                Next
