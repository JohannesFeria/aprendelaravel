...
Partial Class Modulos_Inversiones_Reportes_frmReporteVencimientos
    Inherits BasePage
    ...
    Private Sub GenerarReporte()
        ...
        Try
            ...
            Dim dtVencimientos As DataTable = oOrdenPrevOrden.ReporteVencimientosCuponesOrdenes(ddlPortafolio.SelectedValue, UIUtility.ConvertirFechaaDecimal(tbFechaInicio.Text), ddlreporte.SelectedValue)
            If dtVencimientos.Rows.Count > 0 Then
                ...
                If ddlreporte.SelectedValue = "1" Then
                    Nombre = "ReporteVencimientos-Cupones" & String.Format("{0:yyyyMMdd}", DateTime.Today) & String.Format("{0:HHMMss}", DateTime.Now) & ".xlsx"
                    sFile = New ParametrosGeneralesBM().Listar("RUTA_TEMP", DatosRequest).Rows(0)("Valor") & Nombre
                ElseIf ddlreporte.SelectedValue = "2" Then
                    Nombre = "ReportesVencimientos-Ordenes" & String.Format("{0:yyyyMMdd}", DateTime.Today) & String.Format("{0:HHMMss}", DateTime.Now) & ".xlsx"
                    sFile = New ParametrosGeneralesBM().Listar("RUTA_TEMP", DatosRequest).Rows(0)("Valor") & Nombre
                End If
                Dim Indice As Integer = 1
                If File.Exists(sFile) Then File.Delete(sFile)
                If ddlreporte.SelectedValue = "1" Then
                    sTemplate = RutaPlantillas() & "\" & "ReportesVencimientos-Cupones.xlsx"
                ElseIf ddlreporte.SelectedValue = "2" Then
                    sTemplate = RutaPlantillas() & "\" & "ReportesVencimientos-Ordenes.xlsx"
                End If
                ...
                oCells = oSheet.Cells
                oSheet.SaveAs(sFile)
                DatosExcel(dtVencimientos, 4, oSheet, oCells)
