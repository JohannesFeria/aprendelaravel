...
Imports UIUtility
Imports System.Data
Partial Class Modulos_Inversiones_InstrumentosNegociados_frmListarSwapPorcentaje
    Inherits BasePage
    Dim oPortafolio As New PortafolioBM
    Dim oOrdenInversionBM As New OrdenPreOrdenInversionBM
    Sub CargaGrilla()
        dgLista.DataSource = oOrdenInversionBM.ConsultaOrdenSwapBono(ddlPortafolio.SelectedValue, ConvertirFechaaDecimal(txtFechaOperacion.Text), String.Empty).Tables(0)
        dgLista.DataBind()
    End Sub
    Sub FechaPortafolio()
    ...
    Protected Sub dgLista_RowCommand(sender As Object, e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles dgLista.RowCommand
        Try
            ...
            If e.CommandName = "Modificar" Then
                ...
                'Response.Redirect("frmSwapPorcentaje.aspx?cod=" & e.CommandArgument.ToString() & "&portafolio=" & gvr.Cells(4).Text & "&estado=E-ELI")
                'Else
                Response.Redirect("frmSwapPorcentaje.aspx?cod=" & e.CommandArgument.ToString() & "&portafolio=" & gvr.Cells(4).Text & "&estado=" & gvr.Cells(2).Text)
                'End If
            Else
                If gvr.Cells(10).Text <> UIUtility.ObtenerFechaNegocio(gvr.Cells(4).Text) Then
                    AlertaJS("La fecha de negocio del portafolio es distinta de la orden.")
    ...
    Protected Sub btnIngresar_Click(sender As Object, e As System.EventArgs) Handles btnIngresar.Click
        Response.Redirect("frmSwapPorcentaje.aspx?cod=")
    End Sub
End Class